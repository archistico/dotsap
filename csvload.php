...
if ($handle) {
    ...
    while (($line = fgets($handle)) !== false) {
        ...
        $telefono = pulisciStringaTelefono($parti[2]);
        $eta = $parti[3];
        $rischio = $parti[4];

        $denominazione = $cognome. " " . $nome;

        if ($denominazione_precedente == $cognome. " " . $nome) {
            echo $persona_precedente;
            echo $cognome. " " . $nome. ", ". $telefono . ", ". $eta . ", ". $rischio . "\n";
        }

        $denominazione_precedente = $denominazione;
        $persona_precedente = $cognome. " " . $nome. ", ". $telefono . ", ". $eta . ", ". $rischio . "\n";
...
function pulisciStringaTelefono($text)
{
    if(!empty($text)) {
        $text = str_replace("-", " ", $text);
        $text = str_replace("     ", " ", $text);
